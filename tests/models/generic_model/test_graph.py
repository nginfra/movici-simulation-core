...
from movici_simulation_core.models.generic_model.graph import (
    GenericModelGraph,
    GraphFactory,
    IncompleteSource,
    create_graph,
        ...
        [
            ...
            (
                ...
                lambda _: EntityGroupBlock(
                    dataset="some_dataset", entity_name="some_group", geometry="point"
                ),
            ),
            (
                {
...
class TestGraph:
    @pytest.mark.parametrize("func", ["setup", "initialize", "reset", "update", "validate"])
    def test_graph_proxies_functions(self, func):
        blocks = [Mock(), Mock()]
        graph = GenericModelGraph(blocks)
        graph.outputs = blocks  # required for initialize and update ``to work
        getattr(graph, func)()
