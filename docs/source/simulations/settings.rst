...
its own process. This has the benefit that for certain simulations, some models may calculate in
parallel which speeds up the simulation. It is also a requirement for (future) support of running a
...
- Reduce overhead. Data is not serialized/deserialized between models. Also, in a distributed setup
  the number of processes that are created can sometimes reach 50+ for certain scenarios. This is
