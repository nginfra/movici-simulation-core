...
- ``drinking_water.minor_loss`` (``OPT``): Minor loss coefficient (Default 0). When a pipe has 
  curves and bends, additional head loss may occur, these losses are bundled into a single
...
- ``drinking_water.velocity`` (``PUB``): water velocity (always positive, use flow sign for direction)
