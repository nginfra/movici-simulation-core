...
:ref:`aggregation function<area_aggregation-functions>` and the result is written to the
``target_attribute`` attribute of the target entity.


.. _area_aggregation-functions:
...
When a source entity overlaps multiple target polygons its contribution is divided over every
matching polygon equally. The (relative) overlap over all matched areas is not factored in.
...
| Option             | Type    | Description                                           |
+====================+=========+=======================================================+
| source_entity_group| array   | ``[dataset_name, entity_group_name]`` for the entity  |
|                    |         | that should be aggregated                             |
+--------------------+---------+-------------------------------------------------------+
| source_geometry    | string  | Source geometry type: ``"point"``, ``"line"``, or     |
|                    |         | ``"polygon"``                                         |
+--------------------+---------+-------------------------------------------------------+
| source_attribute   | string  | Attribute to aggregate from ``source_entity_group``   |
+--------------------+---------+-------------------------------------------------------+
...
* When a source entity overlaps multiple target polygons, its contribution is
  split equally between the overlapping targets.
...
* The ``output_interval`` option can be used to control how frequently the
  model produces outputs. It must be set when using any of the ``integral``
