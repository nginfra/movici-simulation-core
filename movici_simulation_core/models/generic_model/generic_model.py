...
class InputBlock(Block):
    ...
    def setup(self, state: TrackedState, **_):
        self.attribute_object = state.register_attribute()
    ...
    def update(self, **_):
        self.has_changes = self.attribute_object.has_changes()
        self.should_update = False
...
class GraphFactory:
    ...
    def create_entity_group_block(self, config):
        ...
        return EntityGroupBlock(dataset=dataset, entity_group=eg_name, geometry=config["geometry"])
...
class GenericModelGraph:
    def __init__(self, blocks: t.Sequence[Block], logger: logging.Logger = None) -> None:
        self.blocks = blocks
        self.logger = logger
        self.inputs_to_outputs = self.map_inputs_to_outputs(blocks)
    ...
    def initialize(self, **kwargs):
        for block in self.blocks:
            block.initialize(**kwargs)

    def update(self, **kwargs):
        self.reset()
        for output in self.get_outputs_to_update(**kwargs):
    ...
    def get_outputs_to_update(self, **kwargs):
        outputs_to_update: t.Set[OutputBlock] = set()
        for input in self.inputs_to_outputs:
            input.update(**kwargs)
            if input.has_changes:
                outputs_to_update.update(self.inputs_to_outputs[input])
        return outputs_to_update

    def validate(self):
        for block in self.blocks:
            block.validate()

        self.detect_dangling_blocks()
        self.detect_duplicate_outputs()

    def detect_dangling_blocks(self):
    ...
    def detect_duplicate_outputs(self):
        all_outputs = [block for block in self.blocks if isinstance(block, OutputBlock)]
        unique_outputs = {(o.entity_group, o.attribute_name) for o in all_outputs}
        if self.logger and len(all_outputs) > len(unique_outputs):
            self.logger.warning("duplicate outputs detected")

    @staticmethod
    def map_inputs_to_outputs(blocks: t.Sequence[Block]):
        inputs_to_outputs = {b: [] for b in blocks if isinstance(b, InputBlock)}
        all_outputs = [b for b in blocks if isinstance(b, OutputBlock)]
        for output in all_outputs:
            inputs = GenericModelGraph.get_sources_for_block(output, block_type=InputBlock)
            for input_block in inputs:
                inputs_to_outputs[input_block].append(output)
    ...
    def get_sources_for_block(
        ...
        for source in sources:
            if block_type is None or isinstance(source, block_type):
                rv.add(source)
