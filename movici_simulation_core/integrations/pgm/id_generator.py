...
class IdGenerator:
    """Bidirectional mapping between Movici IDs and Power Grid Model IDs.

    Unlike aequilibrae which requires sequential IDs starting from 1,
    power-grid-model accepts any int32 IDs. However, we still need to
    maintain mappings for result extraction and updates.

    This implementation uses the Movici Index class for efficient lookups.
    """

    def __init__(self) -> None:
        self.index = Index()

    def register_ids(self, movici_ids: npt.ArrayLike) -> np.ndarray:
        """Register Movici IDs and return corresponding PGM IDs.

        For simplicity, we use sequential IDs (0, 1, 2, ...) as PGM IDs.
        This ensures uniqueness and efficient array indexing.

        :param movici_ids: Array of Movici entity IDs.
        :returns: Array of PGM IDs (0-indexed sequential).
        """
        self.index.add_ids(movici_ids)
        return self.index[movici_ids]

    def get_pgm_ids(self, movici_ids: npt.ArrayLike) -> np.ndarray:
        """Get PGM IDs for given Movici IDs (must be already registered).

        :param movici_ids: Array of Movici entity IDs.
        :returns: Array of PGM IDs.
        :raises ValueError: If any ID is not registered.
        """
        pgm_ids = self.index[movici_ids]
        if np.any(pgm_ids == -1):
            missing = np.asarray(movici_ids)[pgm_ids == -1]
            raise ValueError(f"Movici IDs not registered: {missing}")
        return pgm_ids

    def get_movici_ids(self, pgm_ids: npt.ArrayLike) -> np.ndarray:
        """Get Movici IDs for given PGM IDs.

        :param pgm_ids: Array of PGM IDs (0-indexed).
        :returns: Array of Movici entity IDs.
        :raises ValueError: If any PGM ID is out of range.
        """
        if not isinstance(pgm_ids, np.ndarray):
            pgm_ids = np.asarray(pgm_ids)
        try:
            return self.index.ids[pgm_ids]
        except IndexError as e:
            raise ValueError(f"PGM IDs out of range: {pgm_ids}") from e

    def get_movici_indices(self, pgm_ids: npt.ArrayLike, target_ids: npt.ArrayLike) -> np.ndarray:
        """Get indices into target_ids array for PGM result mapping.

        This is useful when publishing results back to entity groups,
        where we need to map PGM output IDs to entity array indices.

        :param pgm_ids: Array of PGM IDs from calculation results.
        :param target_ids: Array of Movici IDs (entity.index.ids).
        :returns: Array of indices into target_ids.
        :raises ValueError: If Movici IDs not found in target.
        """
        movici_ids = self.get_movici_ids(pgm_ids)
        target_index = Index()
