...
class LegacyTrackedBaseModelAdapter(BaseModel):
    use_numpy = True

    def __init__(self, model: "LegacyTrackedBaseModel", name: str, config: Config):
...
class LegacyTrackedBaseModel:
    ...
    It is also possible to extend this `LegacyTrackedBaseModel` class and let the
    `LegacyTrackedBaseModelAdapter` manage the `TrackedState`

    Attributes:
        auto_reset  By default, the LegacyTrackedBaseModelAdapter resets tracking information of
                    the state for PUB and/or SUB properties at the appropriate time, so that the
                    model receives a SUB update only once, and PUB properties are published
    ...
    def setup(
        ...
        automatically by the `LegacyTrackedBaseModelAdapter`. However, additional datasets may be
        requested directly through the `data_fetcher` parameter.

        :param state: The model's TrackedState object, managed by the
    ...
    def initialize(self, state: TrackedState):
        ...
        another model's t0-update, and the `LegacyTrackedBaseModelAdapter` can wait for this to
        happen before calling `initialize`. When the simulation progresses to `t>0` before the
        model's INIT properties have been filled, an Exception is raised, indicating that the model
        was not ready yet.
...
def model_factory(model_cls: t.Type[LegacyTrackedBaseModel]):
    """Helper function to use in the `run_{model}.py` entry point script. This helper wraps the
    model in a `LegacyTrackedBaseModelAdapter` before giving it over to the model driver. The entry
    point's `execute` function takes the result of this helper: eg.
    `execute(model_factory(MyTrackedModel))`

    :param model_cls: a class (not an instance) inheriting from `LegacyTrackedBaseModel`
    """

    def create_model(name: str, config: Config):
        return LegacyTrackedBaseModelAdapter(model_cls(), name, config)
