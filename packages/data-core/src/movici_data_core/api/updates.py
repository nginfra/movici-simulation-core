...
def request_filetype(accept: t.Annotated[str | None, Header()] = None) -> FileType:
    if not accept or accept == "*/*":
        return FileType.JSON
    return infer_filetype_from_filename_or_mimetype(mimetype=accept)


DepAcceptFileType = t.Annotated[FileType, Depends(request_filetype)]
...
async def create_update(
    backend: DepScenarioBackend, request: Request, filetype: DepAcceptFileType
) -> OperationSuccess:
    ...
    return OperationSuccess(resource="update", id=result, verb="created")
...
async def delete_updates(backend: DepScenarioBackend) -> OperationSuccess:
    ...
    return OperationSuccess(resource="updates", id=backend.scenario_id, verb="deleted")


@update_router.get("/{update_id}")
async def get_update(
    update_id: UUID, backend: DepBackend, filetype: DepAcceptFileType
) -> UpdateWithDataOut:
