...
IdSequence = npt.NDArray[np.int32] | list[int]


class Index:
    def __init__(self, ids: IdSequence | None = None, raise_on_invalid=False):
    ...
    def __getitem__(self, item: int | IdSequence) -> t.Union[int, np.ndarray]:
...
@numba.njit
def query_indices(block_from, block_to, block_offset, ids) -> npt.NDArray[np.int64]:
...
@numba.njit
def query_idx(block_from, block_to, block_offset, ident) -> int:
    not_found = -1
    if len(block_to) == 0:
        return not_found
...
def build_index(ids: IdSequence):
